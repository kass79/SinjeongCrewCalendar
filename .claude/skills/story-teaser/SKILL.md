...
카스는 티저를 **본편 영상 맨 앞에 붙인다.** 그래서 spec에 `"cold_open": true`가
기본 — 엔드카드의 「本編でご確認ください」CTA가 빠지고 제목만 남는다
(이미 본편 안이라 "본편에서 확인"은 어색하다). 별도 영상으로 올릴 때만 뺀다.
...
**화자가 텍스트에 명시되면(1인칭 "私" 등) 명료도보다 화자 일치가 우선이다.**
「妻の手のひら」는 남편 본인이 "私は妻の手を見た"라 말하는 1인칭 회고 —
오디션에서 jf_alpha가 명료도 최고(2.5%)여도, 여성 목소리로 읽으면 화자와 안 맞아
jm_kumo(남성, 0.8%)를 택했다. 대신 선명도 EQ를 표준보다 세게 걸어 보완한다.
...
**`scripts/make_narration.py`가 한 번에** 한다 — 자막 = 나레이션이 기계적으로 보장된다:

```bash
python3 $S/make_narration.py spec.json --dry        # 가나 변환 표 확인 (반드시 눈으로!)
python3 $S/make_narration.py spec.json --out voice  # TTS → voice_clean/ + 믹스 명령 출력
```

변환기(pykakasi)는 85% 정확도라 자주 틀리는 단어(迎え火·提灯·숫자+分 등)는
스크립트 안 `FIX` 사전으로 잡는다. 표에서 틀린 게 보이면 **사전에 추가**하거나
그 자막에 `"kana": "..."`를 넣어 덮어쓴다. 스크립트가 끝에 출력하는 ffmpeg 믹스
명령을 렌더 파일명만 바꿔 그대로 쓴다 (amix 첫 입력은 BGM).
