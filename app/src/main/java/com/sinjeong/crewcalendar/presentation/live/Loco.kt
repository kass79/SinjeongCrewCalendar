 ...
 * 여기에 **예외가 둘** 있다 — 오른변 **위 두 역**([topN]·[topN]+1 = `건대입구`·`구의`)도
 * 기울인다. 카스: *"뚝섬이 한양대 텍스트기울기처럼 노선도 옆에 넣어줘야지.
 * **건대입구도 마찬가지**, 합정, 홍대입구도 노선도 옆에 넣어줘."*(v1.7.15 ⑤) ·
 * *"홍대입구, **건대입구**, 성수 글꼴을 **기울이거나 줄여서** 최적화 시켜줘!"*(v1.7.17 ②)
 *
 * ## 왜 오른쪽 위 모서리만인가
 *
 ...
 * 상대가 `성수`(KEY · 굵게)라 세로 변보다 먼저 자리를 잡기 때문이다. 진 쪽은
 * `SIDE_LANE2` 로 89px 물러났고, 그 물러난 상자가 윗변 `뚝섬` 까지 밀어냈다.
 * 나머지 세 모서리는 세로 변이 먼저 놓여 이긴다.
 *
 * ## 왜 [topN]+1(`구의`)까지인가 — v1.7.17 ②
 *
 * `건대입구` 하나만 기울이면 오른변에서 **혼자만** 누워 어색하고, 그 기울인 상자가 바로 밑
 * `구의` 의 제자리를 물어 `구의` 를 아래로 63px 밀어냈다(접힘 전체 보기 실측 93px → 69px).
 * 둘을 같이 기울이면 **평행한 두 띠**로 겹쳐 앉아 자리를 덜 먹는다 — 오른변 위 두 역이
 * **한 벌**로 읽히고 `강변`·`잠실나루`·`잠실` 은 종전대로 가로다.
 *
...
internal fun labelTilted(
    ...
    return horiz || (!sideLaneOutside && (k == topN || k == topN + 1))
}
 ...
 * *"전체보기 해보니 폴더를 접었을때 펼쳤을때 더 최적화 … **역명이 노선도에 좀 더 붙어야**
 * 할거 같애."*(v1.7.17 ②)
 *
 * 값은 **16dp 한 값**이고, 예외가 딱 하나다 — **접힘 × 전체 보기(복선)의 루프 안쪽**
 * (윗변·세로 변)만 **26dp**.
 *
 * ## 왜 그 한 자리만 26dp 인가 — 모서리가 **1.6px 여유**로 서 있다 (v1.7.17 ② 실측)
 *
 * 복선은 안쪽 선로의 모서리 반지름이 차선 간격만큼 깎여 **거의 직각**이다
 * (접힘 실측 바깥 46dp ↔ 안쪽 **9.9dp**). 그래서 왼쪽 위 모서리에서 `합정`(윗변)과
 * `당산`(좌변)의 이름 상자가 같은 칸을 놓고 다툰다. `합정` 은 거기서 **늘 거울로 눕는데**
 * (제자리 −35° 는 꼬리가 왼쪽 선로를 26px 넘는다), 그 거울 상자의 위 모서리와 `당산`
 * 상자의 아랫날 사이가 **26dp 에서 1.6px** 다. 24dp 면 겹쳐 `당산` 이 둘째 차선으로
 * **153px** 물러나고, 그 상자가 다시 `홍대입구` 를 **160px** 까지 밀어낸다.
 *
 * 나머지 세 자리는 16dp 로도 갈라선다:
 *  · **아랫변**은 이름이 루프 **밖**에 살아 모서리 다툼이 아예 없다(`bottom`).
 *  · **펼침**은 안쪽 모서리 반지름이 **18.9dp** 로 접힘의 두 배다
 *    (실측 43개 평균 95.5px → **53.3px**).
 *  · **단독 보기**는 선로가 한 줄이라 모서리가 46dp 로 넉넉하다.
 *
 * ⚠ 접힘 복선의 26dp 를 줄이려면 **`당산`·`구의` 를 어디에 세울지부터** 정해야 한다 —
 * 거리만 줄이면 둘이 둘째 차선으로 물러나 이름 열을 떠난다(실측표는
 * `docs/project-notes.md` v1.7.17 ②).
 *
 * @param dual 전체 보기(복선)인가 — 단독 보기는 `false`.
 * @param big 폴드 **펼침**인가.
 * @param bottom **아랫변**(루프 밖) 이름인가.
 */
internal fun labelGapDp(dual: Boolean, big: Boolean, bottom: Boolean): Float =
    if (dual && !big && !bottom) 26f else 16f
