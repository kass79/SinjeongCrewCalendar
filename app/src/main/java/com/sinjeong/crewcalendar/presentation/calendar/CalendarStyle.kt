...
enum class CalendarTextSize(val label: String, val factor: Float) {
     ...
     *
     * **규칙(v1.7.17 ① · 되돌리지 말 것): 어느 배율에서든 네 단계는 서로 다른 값이다.**
     * 상한은 **키우는 폭**(`factor - 1`)에만 걸리고 폭이 0 이 되지는 않는다 —
     * 배율이 [MAX_EXTRA_SCALE] 를 넘으면 그 뒤로는 폭이 **더 자라지 않을 뿐**이다.
     * `작게`·`보통` 은 아예 손대지 않는다(줄이는 쪽·기본값은 상한과 무관).
     *
     * 순수 함수 — `CalendarTextSizeTest` 가 잠근다.
     */
    fun factorAt(fontScale: Float): Float =
        if (factor <= 1f) factor
        else 1f + (factor - 1f) * (MAX_EXTRA_SCALE / fontScale.coerceAtLeast(1f)).coerceAtMost(1f)
    ...
    companion object {
        /**
         * **키우는 폭이 시스템 배율을 따라가는 한계 배율 1.5.**
         *
         * 배율이 이 값을 넘으면 `크게`·`아주 크게` 가 더해 주는 폭은 **1.5배에서 멈춘다**
         * (폭이 사라지지는 않는다 — 위 [factorAt] 규칙).
         *
         * ⚠ **v1.7.16 까지는 "곱 상한 1.8"** 이었다. 카스 보고(2026-09-09, 폴드7 을 글자 크게
         * 놓고 씀): *"달력 날짜, 숫자 크기, 달력 메모크기 적용이 잘안되는듯?"* — 실측 결과
         * **배율 1.5 에서 `크게`=`아주 크게`, 배율 1.8 이상에서는 `보통`=`크게`=`아주 크게`** 로
         * 네 단계가 두 개로 뭉개져 **아무리 바꿔도 화면이 안 변했다**(에뮬 로그 실측:
         * fs 2.0 에서 세 단계 모두 `dateSp=8.0 memoSp=11.0` 으로 **비트까지 같은 값**).
         * 곱에 상한을 두는 방식은 **원리적으로** 이 증상을 못 피한다 — `보통` 의 곱은 늘
         * 배율 그 자체라, 곱 상한이 배율에 가까워지면 위 단계들이 `보통` 으로 눌려 붙는다.
         * 그래서 상한을 **폭 쪽으로** 옮겼다.
         *
         * 1.5 인 근거는 종전과 같다 — 출시 점검표가 **깨지지 않음을 매번 확인하는 배율**이다
         * (`SKILL.md` 글자배율 항목). 그 위로는 `보통` 만 시스템을 그대로 따라간다.
         */
        val MAX_EXTRA_SCALE = 1.5f
