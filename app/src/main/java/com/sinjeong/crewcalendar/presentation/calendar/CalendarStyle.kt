...
/**
 * 달력 칸의 **글꼴 크기 단계** — 설정 > 화면 > `달력 날짜 숫자 크기` · `달력 메모 크기` ·
 * `달력 근무 다이아 크기`(v1.7.15 ⑦ · 다이아는 v1.7.18 ③).
 *
 * 카스 원문: *"그리고 달력숫자,메모 글꼴크기 설정할수 있게 해줘"* ·
 * *"달력 날짜 숫자크기와 메모 크기가 큰 차이가 없는듯? 그리고 근무 다이아 크기는 없네?"*(v1.7.18).
 *
 ...
 *
 * ## ⚠ 시스템 글자배율과 **곱해진다** — 그리고 그걸 누르지 않는다
 *
 * 안드로이드 설정에서 1.5배를 쓰는 사람이 `크게` 를 고르면 `1.5 × 1.3 = 1.95` 배다.
 * **v1.7.18 부터 앱은 그 곱을 손대지 않는다** — [factor] 를 그대로 곱하고 배율은 `sp` 가 알아서
 * 얹는다. 시스템 배율은 접근성 설정이라 앱이 누를 자리가 아니고(`LocalDensity` 를 갈아 끼우면
 * dp 까지 따라 움직여 칸 크기가 바뀐다), 무엇보다 **누르면 큰 배율 사용자에게 단계 차이가 가장
 * 작아진다** — 그게 카스가 겪은 증상이다(아래 [Companion] 주석).
 *
...
enum class CalendarTextSize(val label: String, val factor: Float) {
    ...
    /**
     * [base] sp 글자가 이 단계에서 실제로 그려지는 **sp 값**.
     *
     * **규칙(v1.7.18 ③ · 되돌리지 말 것): 시스템 글자배율은 여기에 안 들어온다.**
     * `sp` 가 화면에서 알아서 배율을 얹으므로 이 함수는 [factor] 만 곱한다 —
     * 배율에 따라 배수를 누르는 규칙을 **다시 넣지 말 것**(아래 [Companion] 주석).
     *
     * 순수 함수 — `CalendarTextSizeTest` 가 잠근다.
     */
    fun spOf(base: Float): Float = base * factor

    companion object {
        /**
         * ## ⚠ **배율에 따라 배수를 누르는 규칙은 v1.7.18 에서 삭제됐다** — 되살리지 말 것
         *
         * 카스(2026-09-09, v1.7.17 을 폴드7 에서 시스템 글자 크게 놓고 씀):
         * *"**달력 날짜 숫자크기와 메모 크기가 큰 차이가 없는듯?**"*.
         *
         * v1.7.15~16 은 **곱 상한 1.8**, v1.7.17 은 **폭 상한**(`MAX_EXTRA_SCALE / fontScale`)
         * 이었다. 둘 다 **큰 배율 사용자일수록 단계 차이를 작게** 만든다 — 폭 상한에서도
         * 배율 2.0 이면 `크게` 가 1.15, `아주 크게` 가 1.3 으로 눌려, 정작 이 설정이 가장
         * 필요한 사람이 가장 밋밋한 화면을 봤다(카스가 그 사람이다).
         * 상한을 두는 방식은 **원리적으로** 이 증상을 못 피한다: `보통` 의 곱은 늘 배율
         * 그 자체라, 위 단계를 배율 쪽으로 누르면 `보통` 에 붙는다.
         * 그래서 v1.7.18 은 상한을 **아예 없애고** 단계 폭을 대신 키웠다(1.2/1.4 → **1.3/1.6**).
         *
         * (옛 근거였던 "배율 1.5 는 출시 점검표가 매번 확인하는 배율" 은 그대로다 —
         * 다만 그것은 **점검할 배율**이지 **글자를 눌러야 할 배율**이 아니었다.)
         *
         * 극단(배율 2.0 × `아주 크게` = 3.2배)에서 칸이 모자라는 것은 **알고 두는 대가**다:
         * 근무 칩·출근시각이 먼저고(`MainCalendarScreen` 의 `tightCell`), 메모는 0줄이면
         * 잘림 점만 남는다(v1.7.7 D4 구조 그대로).
         */
        ...
        /**
         * 달력 칸 **근무 다이아 칩** 기본 sp(v1.7.18 ③) — v1.7.17 까지 화면에 박혀 있던 값 그대로다.
         * [DUTY_SP] 는 한두 글자 다이아(`1`·`33`), [DUTY_SP_SMALL] 은 세 글자 이상(`지대11`)과
         * 두 줄 칩의 접두 줄(`대기충당` — 여기에 ×0.7 이 더 붙는다).
         */
        val DUTY_SP = 11.5f
        val DUTY_SP_BIG = 13f
        val DUTY_SP_SMALL = 10f
        val DUTY_SP_SMALL_BIG = 11.5f

        /** 근무 칩 **기본 폭 dp**(v1.6 이래 글자 수와 무관하게 통일). 단계 배수를 곱해 쓴다. */
        val DUTY_CHIP_DP = 34f
        val DUTY_CHIP_DP_BIG = 42f
