...
class FirestoreScheduleRepository @Inject constructor(
    ...
    /**
     * 기기 로컬 설정 — `휴가는 나만 보기` 스위치 하나를 읽는다(v1.7.19).
    ...
    /**
     * 공유 문서 한 장(`rosterOverrides/{사번_날짜}` · 4키). **서버로 나가느냐 마느냐를
     * [sharedDutyRaw] 가 정하는 자리는 여기 한 곳뿐이다** — 새 쓰기 경로를 만들면 반드시 이 함수로.
     *
     * **`null` 이면 `delete`** 한다(= `휴가는 나만 보기` 대상). 이미 올라가 있던 문서를 지워야
     * 동료 화면에 **원래 근무**가 돌아온다 — 안 지우면 스위치를 켜기 전에 올라간 `연차` 가 남는다.
     * 규칙(`firestore.rules`)의 삭제 조건은 `docId == uid_date` 하나라 그대로 통과한다.
     *
     * `originalDutyRaw`(원래 다이아 = `12`·`휴5` …)는 손대지 않는다 — 올리는 경우엔 종전 그대로다.
     */
    private fun publishOverride(
        ...
        val doc = db.collection("rosterOverrides").document("${uid}_$date")
        val shared = sharedDutyRaw(dutyRaw, theme.hideLeave.value)
        if (shared == null) {
            doc.delete()
            return
        }
    ...
    /**
     * **이미 올라간 내 근무변경을 지금 설정대로 맞춘다**(v1.7.19) — **스위치를 바꾼 순간에만**
     * 부른다(설정 화면 한 곳).
     *
     * 대상은 순수 함수 [republishDates] 가 고른다 — **(오늘−31)~미래** 중 숨김 대상인 날
     * (휴가류·직접입력)뿐이다. 켜져 있으면 [publishOverride] 가 그 문서를 **지우고**(동료에게
     * 원래 근무가 돌아온다) 꺼져 있으면 **다시 올린다** — 방향을 따로 계산하지 않는다.
     * 같은 일을 여러 번 해도 결과가 같아 **멱등**이고, 실패는 [runCatching] 이 삼킨다
     * (오프라인이면 Firestore 가 큐에 쌓아 두었다가 알아서 보낸다).
     * 심사 계정([ReviewerAccount])·`visibleToOthers = false` 는 건너뛴다.
     *
     * ⚠ **앱 시작 때 도는 자동 1회는 없다** — 기본값이 **꺼짐**이라(카스: *"원하는 사람만 숨기게
     * 설정해"*) 안 켠 사람은 v1.7.18 과 동작이 완전히 같아야 하고, 켠 사람은 그 순간 이미 맞춰진다.
     * 완료 플래그도 두지 않는다(맞출 것이 없는 상태가 기본이다).
     *
     * ## ⛔ `BuildConfig.DEBUG` 에서는 절대 돌지 않는다 — **영구 가드**(임시 훅 아님)
     * 개발용 에뮬레이터에 로그인된 계정은 **실제 동료**이고 그 기기의 로컬 근무변경은 낡은 값이다.
     * 디버그 빌드가 이 함수를 돌리면 **그 동료의 실제 서버 기록을 지우거나 낡은 값으로 덮어쓴다** —
     * 스위치 한 번 눌러 보는 순간 남의 근무표가 바뀐다. 빼지 말 것(재게시 검증은 릴리즈 빌드·
     * 실기기에서 한다).
     */
    suspend fun republishMasked() {
