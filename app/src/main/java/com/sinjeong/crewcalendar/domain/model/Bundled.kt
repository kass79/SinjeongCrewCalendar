...
object Bundled {
    ...
    /* ── 2026·2027 공휴일 / 기념일 / 절기 ─────────────────────── */

    private fun d(m: Int, day: Int) = LocalDate.of(2026, m, day)
    private fun d27(m: Int, day: Int) = LocalDate.of(2027, m, day)

    /**
     * 법정공휴일: 날짜 빨강 + **휴일 다이아 시각 적용**.
     *
     * ⚠ **이 표 하나가 출근시각을 정한다.** 휴일 다이아를 평일로 읽으면 승무원이 지각한다 —
     * 출근시각([signOn])·열번/행로표([RouteTable])·통상근무 휴무([Pattern.restOnHolidays])·
     * 주52시간·편승 알람·2호선 시간표·위젯이 전부 여기를 본다.
     * 표가 덮는 해는 [HOLIDAY_YEARS]이고, **그 밖의 해는 화면이 "공휴일 정보 없음"이라고 말한다**
     * ([holidayTableCovers]) — 조용히 평일로 계산하지 않는다(v1.6.92 ①).
     *
     * ### 2027년 (v1.6.92에서 추가 · 2026-09-05 확인)
     *
     * 음력 기반(설날·부처님오신날·추석)은 기억으로 쓰지 않고 출처로 확인했다:
     *  · **한국천문연구원 「2027년 월력요항」** 보도(공휴일 72일 · 주5일 119일 · 3일 이상 연휴 10회)
     *    → 설날 **2/7(일)** · 추석 **9/15(수)**, 일요일 겹침 4일(2/7·6/6·8/15·10/3),
     *      토요일 겹침 5일(2/6·5/1·7/17·10/9·12/25).
     *      https://v.daum.net/v/20260629092431271 · https://www.thedailyeconomy.kr/news/articleView.html?idxno=5775
     *  · **대체공휴일 6회**(2/9 설 · 7/19 제헌절 · 8/16 광복절 · 10/4 개천절 · 10/11 한글날 ·
     *    12/27 성탄절). https://www.vegannews.co.kr/news/article.html?no=383666
     *
     * ⚠ **노동절(5/1 토)에는 대체공휴일이 없다.** 근로자의 날은 「근로자의 날 제정에 관한 법률」에
     * 따른 휴일이라 대체공휴일 적용 대상이 아니다 — **5/3(월)은 평일 다이아다.**
     * v1.6.92가 여기에 `대체휴일`을 넣었다가 v1.6.93에서 뺐다. 대체공휴일 7회로 적은 기사가 많으니
     * 다음에 표를 늘릴 때도 이 자리를 의심해라.
     * https://www.gukjenews.com/news/articleView.html?idxno=3631749
     *  · 부처님오신날 **5/13(목)** — 위 두 곳과 https://publicholidays.co.kr/ko/2027-dates/ 대조.
     *
     * **노동절(5/1)·제헌절(7/17)은 2026년부터 관공서 공휴일**이다(「공휴일에 관한 법률」 개정 →
     * 「관공서의 공휴일에 관한 규정」 개정령 2026-04-28 국무회의 의결, 노동절 5/1·제헌절 7/11 시행.
     * https://www.mt.co.kr/policy/2026/04/28/2026042808250174216). 제헌절은 v1.6.16에서 이미
     * **현업 확인으로** 휴일 다이아에 들어와 있었고 법이 뒤따라온 꼴이다. 2027년엔 둘 다 토요일이라
     * 그 날 자체는 어차피 휴일 다이아이고, **딸려 오는 월요일 대체공휴일(5/3 · 7/19)이 새로 생긴다.**
     *
     * **현충일 6/6(일)은 대체공휴일이 없다** — 대체 대상에서 제외돼 있다. 6/7(월)은 평일 다이아다.
     * 규칙 자동적용으로 6/7을 휴일로 적는 달력 사이트가 있으나(publicholidays.co.kr) 월력요항
     * 산식(76−4=72)과 어긋난다. **가장 틀리기 쉬운 자리라 여기 적어 둔다.**
     *
     * **2027년 전국단위 선거는 없다**(중앙선관위 일정: 2026-06-03 지방선거 다음이 2028-04-12 총선).
     * 2026년 표의 `지방선거`(6/3) 같은 항목이 2027년엔 없는 것이 정상이다.
     *
     * ⚠ 2026-05-01 노동절은 **이 표에 없다** — 앱 저장소가 2026-07에 시작해 그 날이 이미 지난
     * 뒤였고, 지난 달은 스냅샷으로 동결되므로 지금 넣어도 출근시각을 바꾸지 않는다. 알고 둔 공백이다.
     */
    val PUBLIC_HOLIDAYS: Map<LocalDate, String> = mapOf(
        ...
        d(12, 25) to "성탄절",
        // ── 2027 (24건 = 월력요항 항목 수와 일치)
        d27(1, 1) to "신정",                                                    // 금
        d27(2, 6) to "설연휴", d27(2, 7) to "설날", d27(2, 8) to "설연휴",        // 토·일·월
        d27(2, 9) to "대체휴일",                                                // 화 (설날이 일요일)
        d27(3, 1) to "삼일절",                                                  // 월 — 대체 없음
        d27(5, 1) to "노동절",                                                  // 토 (대체 없음)
        d27(5, 5) to "어린이날",                                                // 수
        d27(5, 13) to "부처님오신날",                                            // 목
        d27(6, 6) to "현충일",                                                  // 일 — **대체 없음**
        d27(7, 17) to "제헌절", d27(7, 19) to "대체휴일",                        // 토 → 월
        d27(8, 15) to "광복절", d27(8, 16) to "대체휴일",                        // 일 → 월
        d27(9, 14) to "추석연휴", d27(9, 15) to "추석", d27(9, 16) to "추석연휴", // 화·수·목 — 대체 없음
        d27(10, 3) to "개천절", d27(10, 4) to "대체휴일",                        // 일 → 월
        d27(10, 9) to "한글날", d27(10, 11) to "대체휴일",                       // 토 → 월
        d27(12, 25) to "성탄절", d27(12, 27) to "대체휴일",                      // 토 → 월
    )

    /** 공휴일표가 덮는 해의 범위. 표를 늘리면 여기가 저절로 따라온다 */
    val HOLIDAY_YEARS: IntRange =
        PUBLIC_HOLIDAYS.keys.minOf { it.year }..PUBLIC_HOLIDAYS.keys.maxOf { it.year }

    /**
     * 이 날(달)의 공휴일을 표가 알고 있나. **false면 화면이 "공휴일 정보 없음"이라고 말해야 한다** —
     * 표 밖에서는 신정·설날·추석이 조용히 평일로 계산돼 휴일 다이아를 평일로 읽는다(v1.6.92 ①).
     */
    fun holidayTableCovers(date: LocalDate): Boolean = date.year in HOLIDAY_YEARS
    fun holidayTableCovers(month: java.time.YearMonth): Boolean = month.year in HOLIDAY_YEARS
     ...
     * 지선 휴일·토
     *
     * 1·3·6은 **2026-09 개정 안내문**으로 교대 지점이 당겨졌다 — 양천구청 12:55 → **12:41**,
     * 14:55 → **14:41**. 세 다이아가 한 덩어리로 움직인다(지6 전반종료 = 지1 후반시작 = 12:41,
     * 지1 후반종료 = 지3 후반시작 = 14:41). 열번·계는 [RouteTable.BRANCH_DAY_HOLIDAY] 쪽에 같이 있다.
     * 다른 다이아와 평일 표는 무변경 — 옛 값(12:55·14:55)으로 되돌리지 말 것.
     *
     * **2026-09-06 배포 개정 PDF(`지선행로표 휴일(26.9개정)`)와 지1~지8 전수 대조 — 이 표는 전부 일치**(v1.7.8).
     * 출근 = 전반시작 −30분, 퇴근 = 후반종료 규칙도 여덟 다이아 모두 성립했다.
     */
    val BRANCH_HOLIDAY: Map<String, TimeRow> = mapOf(
        "지1" to TimeRow("7:43", "14:41", false, "8:13#10:25", "12:41-14:41"),
        "지2" to TimeRow("7:47", "16:21", false, "8:17#10:15", "14:21-16:21"),
        "지3" to TimeRow("7:52", "16:54", false, "8:22#10:20", "14:41-16:54"),
        "지4" to TimeRow("9:45", "17:14", false, "10:15#12:45", "15:15-17:14"),
        "지5" to TimeRow("9:50", "18:20", false, "10:20#12:21", "16:21-18:20"),
        "지6" to TimeRow("9:55", "18:54", false, "10:25#12:41", "16:54-18:54"),
