...
object BundledTimetable {
    ...
    /**
     * 편승 열차를 고르는 창 — 신도림 출발 **10~20분 전** (v1.6.28 사용자 확정).
     *
     * v1.6.27까지는 19분이었고, 그 1분 때문에 야간 6조합(`37 평평·평휴`, `42 평평·평휴`,
     * `50 휴휴·휴평`)이 "정확히 20분 전" 열차를 놓쳐 알람 없음이었다. 20으로 넓혀 전부 살아난다.
     * **`46 휴평`(21:41 출발 / 앞 열차 21:20 = 21분 전)만 여전히 창 밖이다** — 21로 올리면
     * 걸리지만 사용자 확정값이 20이라 넓히지 않았다.
     *
     * 창을 넓혀도 다른 다이아의 권장시각은 안 바뀐다 — `maxOrNull()`(가장 늦은 편)이라
     * 창의 늦은 쪽 끝([WINDOW_LATE_MIN])이 그대로면 이미 열차가 있던 다이아는 같은 열차를 고른다.
     */
    private const val WINDOW_EARLY_MIN = 20
