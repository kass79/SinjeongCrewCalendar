...
object BundledTimetable {
    ...
    /**
     * 편승 열차를 고르는 창 — 신도림 출발 **10~24분 전** (v1.6.78 사용자 확정).
     *
     * v1.6.27까지 19분 → v1.6.28 20분 → v1.6.29 21분 → v1.6.75 23분 → **v1.6.78 24분**.
     ...
     *
     *  · v1.6.78에서 24분으로 한 칸 더 넓혔다. 사용자 원문: *"평평 39열차 후반알림이 왜 없어?
     *    5시 36분 편승이니까 ..5시31분 알람"*. 평평 39·휴평 37은 후반 **신도림 6:00**인데
     *    양천구청 5:52는 8분 전(하한 미달)이고 그 앞이 **5:36 = 24분 전**이라 23분 창에서
     *    한 끗 차이로 잘려 있었다. **24는 근거 있는 값이다** — 이 창의 상한은 물리 한계가 아니라
     *    "신도림에서 기다릴 수 있는 시간"이고, 실제 간격은 그 시간대 배차간격이 정한다.
     *    5~7시대 배차는 14~17분이라 `10 + (배차−1)` = **최대 26분**까지 구조적으로 나온다
     *    (낮에는 배차 10분이라 19분을 넘는 칸이 아예 없다). 전수 간격 분포도 10~21에 고르게
     *    퍼져 있고 23이 실재하므로 24는 이상치가 아니라 새벽 배차의 자연스러운 꼬리다.
     *
     ...
     * 참이었고 나머지 넷은 아니었다. **남은 4조합과 각각 필요한 창 값**(24분으로도 안 산다):
     *
    ...
    private fun mainNightSecond(num: Int, date: LocalDate): Advice {
        ...
        // **첫 칸이 설명 텍스트여도 그 다음 네 자리 열번을 읽는다** (v1.6.78 — 사용자: *"평평 39열차
        // 후반알림이 왜 없어?"*). 야간표 8칸은 첫 칸에 **누구에게서 넘겨받는지**를 적은 주석이 들어
        // 있고(`"출고열차#6927교대"`·`"33DIA#5925출고교대"`·`"2015열차교대"`·`"군자편승"`),
        // 이 근무가 실제로 처음 잡는 열차는 **그 다음 네 자리 열번**이다. 8칸 전건을 번들 행로표
        // 스캔으로 직접 판독해 확인했고(pp_39·hp_37·ph_35·hp_33·pp_35), 그 열번에 기존 갈래 신호를
        // 그대로 먹이면 스캔과 같은 답이 나온다 — 규칙을 새로 짓지 않고 **읽는 칸만 넓힌 것**이다.
        //
        //  · `"33DIA#5925출고교대·2015"` → **2015**(영업) = 신도림 교대 — pp_39 스캔: 6:00 신도림에
        //    편승 점선이 닿고 그 자리에서 2015를 받는다. 첫 칸만 보면 통째로 꺼졌다.
        //  · `"2015열차교대·5925"` → **5925**(회송) = 신정기지 출고 — hp_33 스캔: 5:52 신정기지 ○출고.
        //    "2015열차교대"는 **넘겨주는 쪽** 주석이라, 여기서 교대를 신도림으로 읽으면 틀린다.
        //  · `"군자편승·군자출고·1936"` → **1936**(19xx) = 군자기지 출고 — pp_35 스캔: 7:39 군자 ○출고.
        //    (평평 35·휴평 43은 **군자 침실**에서 잔다 — v1.6.77 ①에서 사용자가 확정한 그 둘이다.)
        //
        // 네 자리 열번이 아예 없는 칸(`"6:00~6:30 회송"`)은 종전대로 안 건다.
     ...
     *  · **본선 신도림 교대**([SINDORIM_GAP_MIN]) — 양천구청에서 신도림 출발 10~24분 전에
