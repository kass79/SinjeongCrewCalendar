...
object BundledTimetable {
    ...
    /**
     * 편승 열차를 고르는 창 — 신도림 출발 **10~27분 전** (v1.6.79 사용자 확정).
     *
     * v1.6.27까지 19분 → v1.6.28 20분 → v1.6.29 21분 → v1.6.75 23분 → v1.6.78 24분 → **v1.6.79 27분**.
     ...
     *
     *  · v1.6.79에서 **27분**으로 넓혀 마지막 4칸을 켰다. v1.6.78이 "앞 열차(25~27분 전)냐
     *    뒤 열차(8~9분 전)냐"를 사용자에게 물었고, 사용자가 **앞 열차로 확정**했다. 원문:
     *    *"평평 46은, 휴평45는 6시37분 편승이니 6시32분 알람이지 / 평휴 41.휴휴36 은,
     *    6시19분 차니까 6시14분 알람이지"*. 27분은 위 배차 논거의 상한(26분)을 1분 넘는데,
     *    `6:19 → 6:46`은 신도림 쪽 배차가 아니라 **양천구청 휴일표의 6:19 다음 편이 6:37**이라
     *    생긴 간격이다(6:37은 9분 전이라 하한에 걸린다). 즉 "탈 수 있는 열차가 그것뿐"이다.
     *
     * ⚠ **하한 10분은 사용자가 뒤 열차를 거부해서 그대로 둔 것이다 — 9분으로 낮추지 말 것.**
     * 넷 다 뒤 열차(6:54 = 8분 전 / 6:37 = 9분 전)가 하한에 1~2분 모자라 걸려 있었고,
     * 대기시간만 보면 하한 9가 유리하다. 하지만 하한은 **지각 여유를 깎는 값**이고 사용자가
     * 명시적으로 앞 열차를 골랐다. "대기가 22분이나 된다"는 이유로 되돌리면 사용자 확정을
     * 뒤집는 것이다 — 되돌리려면 사용자에게 다시 물을 것.
     *
     ...
     * 참이었고 나머지 넷은 아니었다. **그 4조합이 v1.6.79에서 켜진 넷**이다:
     *
     * | 조합 | 신도림 출발 | 익일 표 | 고른 앞 열차 | 안 고른 뒤 열차 | 알람 |
     * |---|---|---|---|---|---|
     * | `46 평평` · `45 휴평` | 7:02 | 평일 | 6:37 = **25분 전** | 6:54 = 8분 전 | **6:32** |
     * | `41 평휴` · `36 휴휴` | 6:46 | 휴일 | 6:19 = **27분 전** | 6:37 = 9분 전 | **6:14** |
     *
     ...
     * **27분 → 약 22분**. (행로표 편승 점선의 15분은 사업소 출발~인수까지라 주행시간과 다른 값이다.)
     * v1.6.79의 22분 대기는 **사용자가 알고 감수한 값**이다 — 위 ⚠ 참조.
     */
    private const val WINDOW_EARLY_MIN = 27
