...
data class DutyCode(
    ...
    companion object {
         ...
         *
         * `작연차`는 **v1.6.41에서 뺐다**(사용자 확정: "작연차를 그냥 빼"). v1.6.40에서 휴가 계열을
         * 3묶음으로 접을 때 어느 묶음에도 안 들어가 혼자 넷째 줄에 남아 있던 항목이다.
         * ⚠ **[REST_OPTIONS]에는 그대로 둔다** — 이미 저장된 `작연차` 기록이 계속 옅은 붉은색으로
         * 파싱돼야 한다. 여기서 빠지면 새로 고를 수만 없어진다([parse]가 REST를 먼저 본다).
         */
        ...
        /**
         * 근무변경 시트 **화면 묶음** — 상위 칩을 누르면 하위 칩이 펼쳐진다(v1.6.40 사용자 요청).
         * 휴가 계열이 23개 목록에 평평하게 깔려 있어 자주 쓰는 `충당`·`연차`를 찾으려면 스크롤해야 했다.
         *
         * ⚠ **저장값은 하위 코드 그대로다.** `연차` 안에서 `촉연`을 고르면 `촉연`이 저장된다 —
         * 그룹은 화면에만 있고 [parse]·서버·기존 기록은 손대지 않는다.
         *
         * `연차`·`대휴`는 그 자체로도 쓰는 근무코드라 **하위 목록 맨 앞에 자기 자신**이 들어 있다
         * (= 상위 칩은 펼치기 전용, 자기 자신은 첫 하위 칩으로 고른다).
         * `기타휴가`는 묶음 이름일 뿐 근무코드가 아니라서 하위에 자기 자신이 없다 → 저장될 수 없다.
         */
        val CHANGE_GROUPS: Map<String, List<String>> = linkedMapOf(
            "연차" to listOf("연차", "촉연", "가연차"),
            "대휴" to listOf("대휴", "보상", "장휴", "학습", "만휴"),
            "기타휴가" to listOf("청휴", "돌봄휴가", "동행휴가", "병가", "공가"),
        )

        /**
         * 접힌 상태의 시트 목록 = [CHANGE_OPTIONS] 순서 그대로에서 그룹 소속 항목을 상위 이름으로
         * 바꾸고 중복을 뺀 것. 그룹에 안 든 항목(`충당`·`교체`·`교육` …)은 자리가 그대로다.
         * 22칸 → 12칸(3열 4줄)이라 `충당`·`연차`가 스크롤 없이 보인다(v1.6.41 `작연차` 제거로 한 줄 줄었다).
         */
        val CHANGE_TOP: List<String> = CHANGE_OPTIONS
            .map { code -> CHANGE_GROUPS.entries.find { code in it.value }?.key ?: code }
            .distinct()
