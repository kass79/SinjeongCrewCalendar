...
data class DutyCode(
    ...
    /**
     * 색 기준 타입. **`충당` 하나만** 어떤 다이아를 대신 뛰든 **주황([DutyType.FILL])** 으로
     * 보인다 — 달력·동료근무·동료 탭·공유 이미지 색 매핑은 전부 이 값을 쓴다.
     *
     * v1.7.9 이전엔 `충당`·`대기충당`·`교체` 셋이 **대기(노랑)** 였다. 사용자 지적 *"충당으로
     * 변경할때 색상이 대기 색상이랑 비슷하네? 주황색 계열 어때?"* (2026-09-06) 로 셋을 주황으로
     * 갈랐다가, 같은 날 *"대기충당이랑 교체는 그냥 달력 그대로 색깔이어도 될거같은데(주간이면
     * 주간색깔,야간이면 야간색깔로)?"* 로 **`대기충당`·`교체`를 다시 뺐다** — 그 둘은 이제
     * `지근`과 같이 **고른 다이아의 제 색**을 따른다([ORANGE_FILL]).
     ...
     *
     * ⚠ 다이아 없는 맨 `대기충당`·`교체` 칩은 [OVERRIDE_TYPES]가 `STANDBY` 로 파싱하므로
     * **대기 노랑**이다 — 따라갈 다이아가 없으니 제 낱말의 뜻(대기 근무)을 그대로 쓴다.
     * 맨 `지근` 칩이 초록(`BRANCH`)인 것과 같은 규칙이다.
     *
     ...
     */
    val colorType: DutyType get() = if ((fill ?: raw) in ORANGE_FILL) DutyType.FILL else type

    /** 시각표·행로표 조회용 원본. 충당 계열이면 대신 뛰는 다이아 쪽("지3")만 남긴다 */
    val diaRaw: String get() = if (fill != null) raw.substringAfter(' ').trim() else raw

    /**
     * 달력 셀 표시 텍스트 — 지선 **번호 다이아만** "지" 접두사를 떼고 표시(기존 앱 방식).
     * `지대1`·`지휴`·`지휴5`·`지근` 처럼 뒤가 번호가 아닌 것은 **통째로 남긴다**(v1.7.9).
     */
    val display: String
        get() = when {
            ...
            // **지선은 뒤가 번호일 때만 "지"를 뗀다**(v1.7.9). `지1` → `1` · `지14` → `14` 는
            // 번호대가 본선과 갈려(지1~8 · 지10~14) 헷갈릴 일이 없다. 그러나 **낱말 코드는 떼면
            // 뜻이 사라진다** — `지휴` → `휴`(본선 휴무와 같은 글자) · `지근` → `근`.
            // 사용자(2026-09-06): *"지휴로 바꾸었을때 휴<-밖에 안나오네? 지휴로 나오면 좋겠는데?"* ·
            // *"지근이라고 할때도 아직 근무를 모를때 다이아 없이 저장을 하면 그냥 지근<- 이라고
            // 표시되면 좋겠어. 지금은 근 으로만 표시되네?"*
            // 이 한 줄이 `지휴`·`지휴5`·`지근` 셋을 한꺼번에 살린다(각각 따로 가지를 두지 말 것).
            // ⚠ 동료 탭은 **v1.6.53에 이미 같은 이유로** `raw`를 쓰고 있었다(`DutyMatrix.mateLabel`
            //   — *"지휴5가 본선 휴5와 글자가 같았다"*). 달력·위젯·공유 이미지·근무선택 격자만
            //   뒤늦게 맞춘 것이라 이제 두 화면의 표기가 같다.
            // ⚠ 익일 비번(`지13비`)은 위 `POST_NIGHT` 가지가 먼저 `~` 로 가로채 여기 안 온다.
            // ⚠ `raw.length > 1` 이 없으면 깨진 값 `"지"` 한 글자가 **빈 칸**으로 표시된다.
            isBranch && raw.startsWith("지") && raw.length > 1 && raw.drop(1).all(Char::isDigit) ->
                raw.drop(1)
            else -> raw
        }
    ...
    companion object {
         ...
         * 2단계를 거친다. 색만 다르다 → [ORANGE_FILL].
         */
        val FILL_OPTIONS = setOf("충당", "대기충당", "교체", "지근")

        /**
         * [FILL_OPTIONS] 중 **색을 주황([DutyType.FILL])으로 강제**하는 것 — **`충당` 하나뿐이다.**
         *
         * v1.7.9 에서 `충당`·`대기충당`·`교체` 셋으로 넣었다가 같은 날 사용자가
         * *"대기충당이랑 교체는 그냥 달력 그대로 색깔이어도 될거같은데(주간이면 주간색깔,야간이면
         * 야간색깔로)?"* 라고 해서 둘을 뺐다. 그 둘과 `지근`은 **고른 다이아의 제 색**을 따른다
         * (다이아가 없는 맨 칩은 [OVERRIDE_TYPES]의 타입 — `대기충당`·`교체`는 대기 노랑,
         * `지근`은 지선 초록).
         *
         * ⚠ 여기에 항목을 도로 늘리지 말 것 — 위 두 번의 사용자 확정을 뒤집는 일이다.
         */
        private val ORANGE_FILL = setOf("충당")
         ...
         * `휴가는 나만 보기`([sharedDutyRaw])가 **서버에 안 올리는** 낱말들이다.
         *
         ...
         * [sharedDutyRaw] 가 그 판정에 쓴다(직접입력도 휴가류와 같이 숨긴다).
         *
